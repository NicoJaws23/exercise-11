...
Now I will drop the numeric variables litter size and refs by selecting all the variables except for those two.
...
#Age values
relGest <- lm(data = d, `gestation(mo)` ~ `mass(g)`) #Gestation
...
Now I will create boxplots which will plot relative max lifespan, relative newborn mass, and relative weaning mass in relation to order. Based on the graphs, primates have the highest residual lifespan, cetacea has the highest residual newborn mass, and perissodactyla has the highest residual weaning mass.
